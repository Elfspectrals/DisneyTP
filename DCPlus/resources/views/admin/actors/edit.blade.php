<x-app-layout>
    <div class="min-h-screen bg-[#1a1a1a]">
        <div class="max-w-4xl mx-auto px-8 py-12">
            <h1 class="text-4xl font-bold text-white mb-8">Modifier l'acteur</h1>
            ...
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-8">
                ...
                <form method="POST" action="{{ route('admin.actors.update', $actor) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        @if($actor->photo)
                        <div class="mb-3">
                            <img src="{{ Storage::url($actor->photo) }}" alt="{{ $actor->name }}" class="w-32 h-32 object-cover rounded-lg border border-white/20">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="date" name="birth_date" value="{{ old('birth_date', $actor->birth_date ? $actor->birth_date->format('Y-m-d') : '') }}" 
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="bio" rows="4" 
                            class="w-full bg-white/10 border border-white/20 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#0063e5] focus:border-transparent placeholder-gray-400">{{ old('bio', $actor->bio) }}</textarea>
                    </div>
                    <div class="flex gap-4">
                        <button type="submit" class="px-6 py-3 bg-[#0063e5] hover:bg-[#0483ee] text-white font-semibold rounded-lg transition">
                            Mettre à jour
                        </button>
                        <a href="{{ route('admin.actors.index') }}" class="px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/20 text-white rounded-lg font-semibold transition">
                            Annuler
                        </a>
                    </div>
                </form>
